...
void RL::Run(RobotModel& robot_model) {
  ...
  obs_[8] = (robot_model.omega_des_ + omega_com_) * obs_scales_ang_vel_;
#endif
  robot_model.height_des_ += robot_model.delta_height_des_;
  robot_model.height_des_ = std::clamp(robot_model.height_des_, 0.55, 0.72);
  obs_[9] = robot_model.height_des_ * 12.0;
  obs_[10] = 1;
  obs_[11] = robot_model.step_rise_height_des_ * 10;
  obs_[12] = pos[static_cast<int>(Joints::left_hip_roll_joint)] * obs_scales_dof_pos_;
  obs_[13] = pos[static_cast<int>(Joints::left_hip_pitch_joint)] * obs_scales_dof_pos_;
  obs_[14] = pos[static_cast<int>(Joints::left_knee_joint)] * obs_scales_dof_pos_;
  obs_[15] = pos[static_cast<int>(Joints::right_hip_roll_joint)] * obs_scales_dof_pos_;
  obs_[16] = pos[static_cast<int>(Joints::right_hip_pitch_joint)] * obs_scales_dof_pos_;
  obs_[17] = pos[static_cast<int>(Joints::right_knee_joint)] * obs_scales_dof_pos_;
  // 关节速度
  obs_[18] = vel[static_cast<int>(Joints::left_hip_roll_joint)] * obs_scales_dof_vel_;
  obs_[19] = vel[static_cast<int>(Joints::left_hip_pitch_joint)] * obs_scales_dof_vel_;
  obs_[20] = vel[static_cast<int>(Joints::left_knee_joint)] * obs_scales_dof_vel_;
  obs_[21] = vel[static_cast<int>(Joints::left_wheel_joint)] * obs_scales_dof_vel_;
  obs_[22] = vel[static_cast<int>(Joints::right_hip_roll_joint)] * obs_scales_dof_vel_;
  obs_[23] = vel[static_cast<int>(Joints::right_hip_pitch_joint)] * obs_scales_dof_vel_;
  obs_[24] = vel[static_cast<int>(Joints::right_knee_joint)] * obs_scales_dof_vel_;
  obs_[25] = vel[static_cast<int>(Joints::right_wheel_joint)] * obs_scales_dof_vel_;
  obs_.segment(26, 8) = actions_;

  static double step_time = 0.0;     // 相当于 progress_buf * dt
  step_time += robot_model.control_dt;
  // 步态相位
  double period = robot_model.gait_period;                // 0.8
  double phase = std::fmod(step_time, period) / period;
  obs_[34] = std::sin(2 * M_PI * phase);
  obs_[35] = std::cos(2 * M_PI * phase);

  // 左腿/右腿是否应该抬起
  double phase_left = phase;
  double phase_right = std::fmod(phase + 0.5, 1.0);
  double stance_ratio = robot_model.stance_ratio;         // 0.55
  obs_[36] = float(phase_left >= stance_ratio);          // 左腿 swing
  obs_[37] = float(phase_right >= stance_ratio);         // 右腿 swing



  // static double time_obs = 0;
  // obs_[28] = (obs_[11] != 0) && (robot_model.height_fb_ < 0.6);//(self.cmd_time_buf == 0)
  // obs_[29] = (obs_[11] != 0) && (time_obs < 0.4) && (obs_[28] != 1);
  // obs_[30] = (obs_[11] != 0) && (obs_[28] != 1) && (obs_[29] != 1);
  // if(obs_[28] == 0) time_obs += robot_model.control_dt;
  // else time_obs = 0;
  ...
  // 在仿真中添加延迟,部署时无须添加
  // 缓存队列，长度为20，先进先出
  int kDelayLen = int(0.020 / robot_model.control_dt);
  if (kDelayLen < 1) {
    kDelayLen = 1;
  }

  const int kNumJoints = robot_model.pino_model().nv - 6;
  using ActVec = Eigen::VectorXd;

  static bool delay_initialized = false;
  static std::deque<ActVec> actions_fifo;

  if (!delay_initialized && has_new_action) {
    actions_fifo.assign(kDelayLen, actions_);
  ...
  if (delay_initialized) {
    ...
    if (actions_fifo.size() > static_cast<size_t>(kDelayLen)) {
...
void RL::LoadParameters(RobotModel& robot_model) {
  robot_model.jump_time_window = config_["control"]["jump_time_window"].as<int>();
